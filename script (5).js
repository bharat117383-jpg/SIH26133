...
(function () {
  ...
  function autoDetectLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        const recBanner = document.getElementById("recommendedLangBanner");
        if (recBanner) {
          recBanner.style.display = "block";
          recBanner.innerHTML = `<strong>${getTranslation("lang.recommendedTag")}</strong> ${langObj.native} (${langObj.name}) <button id="btnApplyRec" class="btn btn-sm btn-primary ml-2">${getTranslation("lang.applyRecommended")}</button>`;
          document.getElementById("btnApplyRec")?.addEventListener("click", () => {
  ...
  function bindEvents() {
    ...
    document.getElementById("btnDetectLocation")?.addEventListener("click", autoDetectLocation);
