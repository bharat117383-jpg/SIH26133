...
(function () {
  ...
  function selectLanguage(langCode, announce = true) {
    ...
    // Apply translated text without changing existing application behavior.
    if (typeof window.applySiteLanguage === "function") {
      window.applySiteLanguage(langCode);
    }

    // Re-render dynamic dashboard content so labels are translated too.
  ...
  function updateDashboardUI() {
    ...
    if (p) {
      ...
      if (dashProfileDetails) {
        const tr = (value) => (typeof window.SW_T === "function" ? window.SW_T(value) : value);
        const genderLabel = tr(String(p.gender || ""));
        dashProfileDetails.textContent = `${tr("Age")}: ${p.age} · ${genderLabel} · ${p.city}, ${p.state} · ${tr("Mobile")}: +91 ${p.phone}`;
      }
      if (dashAvatar) {
    ...
    if (dashProfileLang) {
      const tr = (value) => (typeof window.SW_T === "function" ? window.SW_T(value) : value);
      dashProfileLang.textContent = `${tr("Active Language:")} ${currentLangObj.native} (${currentLangObj.name})`;
    }
      ...
      if (APP_STATE.lastAssessment) {
        ...
        dashAssessmentContent.innerHTML = `
          <div class="assessment-results-card">
            <div class="res-row">
              <span class="res-label">${tr("Assessment Date:")}</span>
              <strong class="res-value">${new Date(a.date).toLocaleDateString()}</strong>
            </div>
            <div class="res-row">
              <span class="res-label">${tr("Reported Symptoms:")}</span>
              <strong class="res-value">${a.symptoms.map(tr).join(", ")}</strong>
            </div>
            <div class="res-row">
              <span class="res-label">${tr("Severity & Duration:")}</span>
              <strong class="res-value">${tr(a.severity)} · ${tr(a.duration)}</strong>
            </div>
            <div class="res-row">
              <span class="res-label">${tr("Triage Recommendation:")}</span>
              <strong class="res-value text-teal">${a.specialty}</strong>
            </div>
            <div style="margin-top: 1rem; display: flex; gap: 0.75rem;">
              <button type="button" class="btn btn--primary btn--sm" data-screen-trigger="assessment">${tr("Retake Assessment")}</button>
              <button type="button" class="btn btn--gold btn--sm" data-screen-trigger="doctors">${tr("Consult Doctor")}</button>
            </div>
          </div>
        `;
      } else {
        ...
        dashAssessmentContent.innerHTML = `
          <div class="empty-state">
            ...
            <p>${tr("No health assessment recorded on this device yet.")}</p>
            <button type="button" class="btn btn--primary btn--sm" data-screen-trigger="assessment">
              ${tr("Take Preliminary Assessment")}
            </button>
          </div>
        `;
      }
